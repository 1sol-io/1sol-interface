...
import { addLiquidity, removeLiquidity, useOwnedPools } from "../../utils/pools";
...
export const MigrationModal = () => {
  ...
  const poolsToMigrate = useMemo(() => {
    return legacyOwned.map(item => {
      const account = item.account;
      const fromPool = item.pool;

      const toPool = activePools.pools.find(p =>
        (p.pubkeys.holdingMints[0].equals(fromPool.pubkeys.holdingMints[0]) && p.pubkeys.holdingMints[1].equals(fromPool.pubkeys.holdingMints[1])) ||
        (p.pubkeys.holdingMints[0].equals(fromPool.pubkeys.holdingMints[1]) && p.pubkeys.holdingMints[1].equals(fromPool.pubkeys.holdingMints[0])))
  ...
  const handleOk = useCallback(async () => {
    ...
    for (let i = 0; i < poolsToMigrate.length; i++) {
      try {
        ...
        const poolMint = await cache.queryMint(connection, fromPool.pubkeys.mint);
        ...
        const baseAmount = Math.floor(baseAccount.info.amount.toNumber() * ownershipRatio);
        const quoteAmount = Math.floor(quoteAccount.info.amount.toNumber() * ownershipRatio);

        const toAccounts = await removeLiquidity(connection, wallet, liquidityAmount, account, fromPool);

        const sameMintOrder = fromPool.pubkeys.holdingMints[0].toBase58()  === toPool?.pubkeys.holdingMints[0].toBase58(); 
        if(!sameMintOrder) {
          toAccounts.reverse();
        }
  ...
  return (
    <Modal
      ...
      closable={!executing}
      footer={[
        <Button key="back" onClick={handleCancel} disabled={executing || completed}>
          Ask me later
      </Button>,
        <Button key="submit" type="primary" loading={executing} disabled={completed} onClick={handleOk}>
          Migrate
      </Button>,
      ]}
    >
    ...
    >
      ...
      <>
      <p>You are identified as liquidity provider that used v1 of Serum Swap.</p>
      <p>Please click migrate button to move to new version of the contract</p>
